...
def get_location(index, app, id, xpath, evaluate):
    sta = ''
    bounds = ''
    re_xpath = {'class':"", 'cd':""}    
    xpath_type = ''
        ...
        for root, dirs, files in os.walk(mydir):
            for file in files:
                if file.endswith(".xml") or file.endswith(".uix"):
                    ...
                    if (targetFilePath not in sta_list):
    ...
    if len(map) == 0:
        print("找不到界面", index, app, id, xpath, sta, evaluate)
        return sta, bounds, cnt
...
def get_same_bool(i, j, x):
    ...
    if  (pd.isna(x.loc[x['index']==i, 'tgt_index'].values[0])) and (pd.isna(x.loc[x['index']==i, 'tgt_content'].values[0])) and (pd.isna(x.loc[x['index']==i, 'predict_action'].values[0])):
       return False
    return True

def get_empty_bool(i, x):
    #Tgt_add_id and tgt_add_xpath and tgt_text and tgt_content 都为空的时候不给tgt_index 
    if (pd.isna(x.loc[x['index']==i, 'tgt_add_id'].values[0])) and (pd.isna(x.loc[x['index']==i, 'tgt_add_xpath'].values[0])) and (pd.isna(x.loc[x['index']==i, 'tgt_text'].values[0])) and (pd.isna(x.loc[x['index']==i, 'tgt_content'].values[0])):
        return True
    return False
...
def single_state_fill(x, dict, dict_max, type):
    ...
    tgt_index_column = x.columns.get_loc("tgt_index")
    ...
    for i in range(len(x)-1, -1, -1):
        ...
        if pd.notna(x.iloc[i]['tgt_index']):
            continue
        ...
        l = i  
        r = i
        while pd.isna(x.iloc[l, x.columns.get_loc('tgt_index')]):
            if (l - 1 < 0):
                break
            l = l - 1
        while pd.isna(x.iloc[r, x.columns.get_loc('tgt_index')]):
            if (r + 1 == len(x)):
                break
            r = r + 1
       
        if pd.isna(x.iloc[l, x.columns.get_loc('tgt_index')]):
            #只有右侧
            if (pd.notna(x.iloc[r, x.columns.get_loc('tgt_index')])):
                ...
                while ((index in dict.keys()) or ((index > -3) and (index < 0))):
                    ...
                    if index in dict.keys():
                        if get_same_bool(x.iloc[i]['index'], dict[index], x):
                            break
                    index = index - 1
                x.iloc[i, tgt_index_column] = index
            ...
            if get_screen_bool(i, l, x) or not get_screen_bool(i, l, x):
                ...
                if (pd.notna(x.iloc[r, x.columns.get_loc('tgt_index')])):
                    ...
                    if get_same_bool(x.iloc[l]['index'], x.iloc[r]['index'], x):
                        ...
                        index = (pd.to_numeric(x.iloc[l]['tgt_index']) + pd.to_numeric(x.iloc[r]['tgt_index'])) / 2   
...
def assign_state_order_improved(x):
    ...
    state_list = []
    for i in x.index:
        if (pd.notna(x.loc[i]['tgt_index'])) or (get_empty_bool(x.loc[i]['index'], x)):
            continue
        ...
        if (x.loc[i]['state_name']) and (x.loc[i]['state_order'] == sys.maxsize):
            ...
            for app in relevant_rows:
                ...
                if pd.isna(prev_state_order) and pd.isna(next_state_order):
                    continue
                if (prev_state_order == next_state_order):
                    next_state_order = np.nan
                if pd.isna(prev_state_order):
    ...
    min_state_order_index = x['state_order'].idxmin()
    min_state_name = x.loc[min_state_order_index, 'state_name']
    min_state_order = x.loc[min_state_order_index, 'state_order']
    state_list.append(min_state_name)

    state_counts = x['state_name'].value_counts().to_dict()
    min_state_count = state_list.index(min_state_name)

    
    increment = 0.1
    for i, item in enumerate(state_list):
        if (item == min_state_name):
            continue
        chosen_order = min_state_order + (min_state_count - i) * 0.1
        result_x.loc[x['state_name'] == item, 'state_order'] = chosen_order
...
def fill_pos(x, dict):
    ...
    if len(x.groupby(['src_app'], group_keys=True).groups) == 1:
        ...
        for i in x.index:
            ...
            if str(x.loc[i]['ori_tgt_index']) != 'nan':
                ...
                if (x.loc[i]['state_name'] not in order.values()):
                    order[step] = x.loc[i]['state_name']
                    x.loc[x['state_name'] == x.loc[i]['state_name'], 'state_order'] = step
                    step += 1
    ...
    if len(dict) == 0:
        ...
        if flag_tgt_index == 0:
            for i in x.index:
                if pd.notna(x.loc[i]['ori_tgt_index']):
                    x.loc[i, 'tgt_index'] = x.loc[i]['ori_tgt_index']
                    x.loc[i, 'state_order'] = 1
...
def solve(x):
    ...
    num = num + 1
    for i in x.index:
        x.loc[i, 'state_name'], x.loc[i, 'bounds'], x.loc[i, 'cnt'] = get_location(x.loc[i]['index'], x.loc[i]['tgt_app'], str(x.loc[i]['tgt_add_id']), str(x.loc[i]['tgt_add_xpath']), str(x.loc[i]['evaluate_ori']))
...
def main():
    ...
    selected_columns_df.to_csv("result_atm.csv", index=False)
